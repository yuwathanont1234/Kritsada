...
> [!CAUTION]
> **CRITICAL DATA SAFETY RULE:** **DO NOT** use generic stock-photo websites (such as Unsplash, Pexels, Pixabay) to download watch references. Stock-photo watch images often contain complex visual noise, non-studio background elements, and variations that do not match official manufacturer specifications. Ingesting them causes high-confidence visual vector clustering anomalies, leading to severe RAG "database self-poisoning."
>
> All watch reference embeddings **MUST** exclusively originate from official brand catalogs, authoritative manufacturer studio-grade photos, or authorized licensed brand dealer assets (e.g., licensed Chrono24, Watchbox).

**Do:**
- Build a pipeline script (e.g., `scripts/ingest-real-embeddings.ts`) to ingest real image embeddings for your watch references:
  1. Retrieve all authentic watch reference images from official, authorized manufacturer studio sources. **Strictly forbid Unsplash or stock photos.**
...
**Acceptance:** `image_embeddings` table contains valid, non-random vectors matching DINOv3 dimensions derived ONLY from official manufacturer/authorized brand photos, allowing actual watch scans to successfully hit the database with high similarity (> 0.90) and high spread (> 0.15) while maintaining catalog purity.
