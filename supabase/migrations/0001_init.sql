...
create unique index if not exists watch_embeddings_idem
    on watch_embeddings (brand, ref, source_url, embedding_version);

-- Retrieval index. Filter by embedding_version at query time, then ANN search.
create index if not exists watch_embeddings_ann
    on watch_embeddings using hnsw (embedding vector_cosine_ops);

create index if not exists watch_embeddings_lookup
    on watch_embeddings (brand, ref, embedding_version);
